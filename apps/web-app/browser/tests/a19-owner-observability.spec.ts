...
test('production SPA與canonical ASGI完成A19 two-owner zero-raw browser closure', async ({ browser }) => {
  ...
  await expect.poll(() => endpointResponses(observedA.responses, ENDPOINT_B)).toEqual([
    `404 /api/published-endpoints/${ENDPOINT_B}/diagnostics?window_seconds=86400&limit=50`,
  ...
  await expect(pageB.getByRole('alert')).toHaveText('找不到這個端點。')
  await expect.poll(() => endpointResponses(observedB.responses, ENDPOINT_A)).toEqual([
    `404 /api/published-endpoints/${ENDPOINT_A}/diagnostics?window_seconds=86400&limit=50`,
