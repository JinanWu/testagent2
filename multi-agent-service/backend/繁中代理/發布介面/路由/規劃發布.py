...
class 端點建立服務(Protocol):
    """規範 canonical Endpoint Create route 唯一需要的發布操作。

    方法：``原子發布`` 接收 session owner 與 detached confirmation。
    例外：實作者可傳出控制流程例外；一般結果由路由固定映射。
    副作用：協定本身無副作用；實作者可發布套件、憑證與端點圖形。
    """

    def 原子發布(
        self, *, 擁有者使用者識別碼: str, 確認: 發布確認,
    ) -> 端點發布結果 | 管理操作錯誤:
        """以權威 owner 執行單一原子端點建立操作。

        參數：擁有者識別碼來自 canonical session；確認來自嚴格 body。
        回傳：成功收據或固定管理操作錯誤。
        例外：控制流程例外可原樣傳出。
        副作用：由實作者定義完整發布交易。
        """
...
def 建立安全草稿端點建立路由器(
    ...
    *,
    ...
    """建立 canonical Draft 與 Endpoint Create 兩條安全管理路由。

    參數：接收草稿與發布服務、目前工作階段相依、跨站請求偽造防護相依及可替換時鐘。
    回傳：只含草稿與端點建立兩條 POST 路由的 ``APIRouter``。
...
def 建立安全規劃發布路由器(
    ...
    *,
    ...
    """建立既有 Draft、Create 與 Version 三條安全管理路由。

    參數：接收草稿與發布服務、目前工作階段相依、跨站請求偽造防護相依及可替換時鐘。
    回傳：重用 canonical 兩路由工廠後，再附加版本 POST 的 ``APIRouter``。
    例外：路由註冊失敗時傳遞例外；請求期間錯誤由共用處理器固定映射。
    副作用：建立路由器並註冊三條路由；不在建構期間呼叫任何服務。
    """
    路由器 = 建立安全草稿端點建立路由器(
        草稿服務, 發布服務, 目前工作階段相依, csrf相依, 時鐘=時鐘,
    )
...
def _安全發布端點(服務: 端點建立服務, 請求: 發布端點請求, 使用者識別碼: str) -> JSONResponse:
...
def _呼叫服務(服務: object, 方法名稱: str, 重建器=None, 重建參數=(), **參數):
