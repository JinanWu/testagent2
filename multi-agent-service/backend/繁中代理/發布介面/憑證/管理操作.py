...
def 列出管理憑證(資料庫, 時鐘, 端點識別碼, 擁有者識別碼, 是否管理者=False):
    """不持有管理service或封套的清單boundary。

    ``是否管理者`` 為 True 時略過 owner 比對，讓管理者讀得到別人端點的憑證摘要
    （與 端點查詢、Owner觀測 對管理者的處理一致）。預設 False 是 fail-closed：
    沒有明確傳入就維持 owner-only，呼叫端漏傳不會意外放寬範圍。
    """
    ...
    try:
        ...
        游標 = 連線.execute(
            ...
            "WHERE e.id=? AND (?=1 OR e.owner_user_id=?) ORDER BY c.created_at DESC,c.id ASC",
            (端點識別碼, int(是否管理者), 擁有者識別碼),
        )
