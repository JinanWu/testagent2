...
def test_A22_browser_glue完整且不啟用artifact_capture():
    ...
    assert "reuseExistingServer: false" in config and "建立CP4SPAASGI應用程式" in server
...
def test_A22_server與checker可由隔離Python編譯():
    ...
    result = subprocess.run([sys.executable, "-c", code, *map(str, files)], cwd=REPOSITORY_ROOT, env={"PATH": ""}, capture_output=True, text=True)
