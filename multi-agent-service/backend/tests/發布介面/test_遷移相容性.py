...
def test_fresh_empty_db_apply_0001_to_0011_and_idempotent(tmp_path):
    ...
    assert 初始化發布介面資料庫(db) == tuple(range(1, 17))
...
def test_0006無損保留legacy_audit_row並升級完整事件欄位(tmp_path):
    ...
    assert 初始化發布介面資料庫(db) == tuple(range(6, 17))
    assert _q(
        db,
...
def test_loader_missing_db_is_read_only_and_returns_all_pending(tmp_path):
    ...
    assert [項目.版本 for 項目 in pending] == list(range(1, 17))
    assert not db.exists()
