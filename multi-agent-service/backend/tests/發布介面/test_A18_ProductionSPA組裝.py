...
def test_legacy_production環境dist設定不影響API_only_root_factory(tmp_path: Path, monkeypatch):
    """舊 SPA 設定可被遷移環境接受，但 root factory 不再供應 HTML。"""
    ...
    app = asgi模組.建立環境應用程式()
    with TestClient(app, raise_server_exceptions=False) as client:
        response = client.get("/admin/invocations")
        assert response.status_code == 404
        assert response.headers["content-type"].startswith("application/json")
    del web, published
