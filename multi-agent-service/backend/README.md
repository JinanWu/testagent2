...
現行 Nginx proxy 模式與舊版 Agent Studio 相同：前端不會在 bundle 內暴露
backend URL，但 Cloud Run backend 本身仍是可直接呼叫的服務。若要限制 direct
access，需另行導入 Load Balancer/IAM BFF，不能只修改 React 或 Nginx。
...
啟動前必須提供 `TESTAGENT2_DB_PATH`、`TESTAGENT2_PUBLISHED_DB_PATH`、`TESTAGENT2_PUBLISHED_BUNDLE_ROOT`、`TESTAGENT2_WEB_ORIGINS`、`TESTAGENT2_MODEL_NAME`、`AIAGENT_GCP_PROJECT` 與 `AIAGENT_GCP_LOCATION`。前三個持久路徑不得互為別名。另須由部署secret authority注入兩個 credential keyring 設定：`TESTAGENT2_PUBLISHED_CREDENTIAL_ACTIVE_KEY_VERSION` 與 `TESTAGENT2_PUBLISHED_CREDENTIAL_KEYS_JSON`。Canonical parser 固定正式 provider 為 `gemini-adc`；不得使用 `TESTAGENT2_WEB_DB_PATH` 或 `TESTAGENT2_BUNDLE_ROOT` 別名。Secret 不得保存於 repository、文件範例、log 或 shell 歷史紀錄。
