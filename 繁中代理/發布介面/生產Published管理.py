...
from .路由.規劃發布 import 發布確認, 端點發布結果, 管理操作錯誤, 版本建立結果
...
class 延遲發布管理服務:
    ...
    def 原子建立並切換版本(
        self, *, 擁有者使用者識別碼: str, 端點識別碼: str, 配置: dict,
    ) -> 版本建立結果 | 管理操作錯誤:
        """以單一 active lease 委派不可變 Version 建立與 current pointer 切換。

        參數：權威擁有者識別碼、路徑端點識別碼及安全路由重建的版本配置原樣傳入。
        返回值：協調器的版本建立收據或固定管理操作錯誤。
        例外：服務不可用固定失敗；協調器控制流程例外原樣傳出。
        副作用：租借同一 management slot，可能發布套件並提交新版本與 current pointer。
        """
        with self._租借服務() as 服務:
            return 服務.原子建立並切換版本(
                擁有者使用者識別碼=擁有者使用者識別碼,
