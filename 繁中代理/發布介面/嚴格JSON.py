...
def 解析嚴格JSON(原始文字: str) -> Any:
    ...
    try:
        ...
        raise
    except (json.JSONDecodeError, ValueError) as 錯誤:
        raise 嚴格JSON錯誤(f"JSON 語法或數值不符合嚴格契約: {錯誤.__class__.__name__}") from 錯誤
...
def 建立正規JSON(資料: Any) -> str:
    ...
    try:
        ...
        raise 嚴格JSON錯誤("資料不符合正規 JSON 契約") from 錯誤
