...
def test_append_only不覆蓋既有(假技能環境):
    技能使用事件.記錄事件("demo", "alice")
    技能使用事件.記錄事件("demo", "alice")
    技能使用事件.記錄多筆事件(["a", "b"], "bob")
    assert len(技能使用事件.讀取所有事件()) == 4


def test_記錄多筆事件去重由呼叫端負責(假技能環境):
    # 模組不主動去重；呼叫端傳幾個就寫幾筆
    寫入 = 技能使用事件.記錄多筆事件(["x", "y", "z"], "alice")
    assert 寫入 == 3


def test_空清單或空skill_id不寫入(假技能環境):
    assert 技能使用事件.記錄多筆事件([], "alice") == 0
    assert 技能使用事件.記錄事件("", "alice") == 0
    assert 技能使用事件.讀取所有事件() == []


def test_彙總計數並取最新使用時間(假技能環境):
    技能使用事件.記錄事件("demo", "alice", used_at="2026-07-01T00:00:00+00:00")
    技能使用事件.記錄事件("demo", "alice", used_at="2026-07-05T00:00:00+00:00")
    技能使用事件.記錄事件("demo", "alice", used_at="2026-07-03T00:00:00+00:00")
    彙總 = {(列["user_id"], 列["skill_id"]): 列 for 列 in 技能使用事件.彙總()}
    列 = 彙總[("alice", "demo")]
...
def test_彙總依user_id與skill_id分列(假技能環境):
    ...
    技能使用事件.記錄事件("shared-name", "alice")
    技能使用事件.記錄事件("shared-name", "alice")
    技能使用事件.記錄事件("shared-name", "bob")
    彙總 = {(列["user_id"], 列["skill_id"]): 列 for 列 in 技能使用事件.彙總()}
...
def test_毀損行被略過(假技能環境):
    技能使用事件.記錄事件("demo", "alice")
    with open(技能使用事件.事件檔路徑(), "a", encoding="utf-8") as f:
        f.write("{ 這行壞掉\n")
    技能使用事件.記錄事件("demo2", "alice")
    事件清單 = 技能使用事件.讀取所有事件()
    assert {e["skill_id"] for e in 事件清單} == {"demo", "demo2"}


def test_持久化為jsonl每行一筆(假技能環境):
    技能使用事件.記錄多筆事件(["a", "b"], "alice")
    行清單 = 技能使用事件.事件檔路徑().read_text(encoding="utf-8").splitlines()
    assert len(行清單) == 2
