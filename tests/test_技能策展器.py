...
def test_彙總不重建已刪除技能的殭屍記錄(假環境):
    ...
    assert "sid-gone" not in 技能使用量.讀取全部使用量()
...
def test_閒置技能被標記為stale(假環境, monkeypatch):
    ...
    技能使用量.設定彙總("sid-1", 1, (datetime.now(timezone.utc) - timedelta(days=40)).isoformat())
    統計 = 技能策展器.套用生命週期轉移()
    assert 統計["marked_stale"] == 1
    assert 技能使用量.取得記錄("sid-1")["state"] == 技能使用量.狀態_閒置


def test_久未使用技能被封存並搬到archive(假環境, monkeypatch):
    monkeypatch.setenv("TESTAGENT2_SKILL_ARCHIVE_DAYS", "60")
    _建立技能(假環境, "demo", "sid-1")
    技能使用量.設定彙總("sid-1", 1, (datetime.now(timezone.utc) - timedelta(days=90)).isoformat())
    統計 = 技能策展器.套用生命週期轉移()
...
def test_pin住的技能永不轉移(假環境, monkeypatch):
    monkeypatch.setenv("TESTAGENT2_SKILL_ARCHIVE_DAYS", "60")
    _建立技能(假環境, "demo", "sid-1")
    技能使用量.設定彙總("sid-1", 1, (datetime.now(timezone.utc) - timedelta(days=999)).isoformat())
    技能使用量.設定pin("sid-1", True)
    統計 = 技能策展器.套用生命週期轉移()
    assert 統計["skipped_pinned"] == 1
    assert 統計["archived"] == 0
    assert 技能使用量.取得記錄("sid-1")["state"] == 技能使用量.狀態_使用中
...
def test_stale技能又被使用會復活(假環境, monkeypatch):
    ...
    _建立技能(假環境, "demo", "sid-1")
    技能使用量.設定彙總("sid-1", 1, (datetime.now(timezone.utc) - timedelta(days=40)).isoformat())
    技能使用量.設定狀態("sid-1", 技能使用量.狀態_閒置)
    # 最近又用過 → 錨點變新 → 復活
    技能使用量.設定彙總("sid-1", 2, datetime.now(timezone.utc).isoformat())
    統計 = 技能策展器.套用生命週期轉移()
