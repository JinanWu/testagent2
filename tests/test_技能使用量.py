...
def test_初始化記錄建立預設值並帶user_id(假技能環境):
    技能使用量.初始化記錄("sid-1", user_id="alice")
    記錄 = 技能使用量.取得記錄("sid-1")
    assert 記錄["use_count"] == 0
    assert 記錄["last_used_at"] is None
    assert 記錄["state"] == 技能使用量.狀態_使用中
...
def test_pin設定與查詢(假技能環境):
    assert 技能使用量.是否pin("sid-1") is False
    技能使用量.設定pin("sid-1", True)
    assert 技能使用量.是否pin("sid-1") is True
    技能使用量.設定pin("sid-1", False)
    assert 技能使用量.是否pin("sid-1") is False


def test_設定狀態只接受有效值(假技能環境):
    技能使用量.設定狀態("sid-1", 技能使用量.狀態_封存)
    assert 技能使用量.取得記錄("sid-1")["state"] == 技能使用量.狀態_封存
    技能使用量.設定狀態("sid-1", "不存在的狀態")
    assert 技能使用量.取得記錄("sid-1")["state"] == 技能使用量.狀態_封存  # 未被改動


def test_遺忘移除記錄(假技能環境):
    技能使用量.記錄使用("sid-1")
    assert "sid-1" in 技能使用量.讀取全部使用量()
    技能使用量.遺忘("sid-1")
    assert "sid-1" not in 技能使用量.讀取全部使用量()


def test_使用量報告以skill_id為key並帶name(假技能環境):
    _建立技能檔(假技能環境, "used-skill", "sid-used")
    _建立技能檔(假技能環境, "never-used", "sid-never", category="infra")
    技能使用量.初始化記錄("sid-used", user_id="alice")
    技能使用量.設定pin("sid-used", True)
    報告 = {列["skill_id"]: 列 for 列 in 技能使用量.使用量報告()}
...
def test_使用量報告略過無skill_id的技能(假技能環境):
    ...
    目錄.mkdir()
    (目錄 / "SKILL.md").write_text("---\nname: no-id\ndescription: d\n---\n\n# T\n步驟。\n", encoding="utf-8")
    assert all(列["name"] != "no-id" for 列 in 技能使用量.使用量報告())
...
def test_毀損檔案回傳空dict(假技能環境):
    技能使用量.使用量檔路徑().write_text("{ 壞掉的 json", encoding="utf-8")
    assert 技能使用量.讀取全部使用量() == {}
