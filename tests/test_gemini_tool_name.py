    ...
    def 產生回應(self, 訊息清單, 工具清單):
        ...
        if self.呼叫次數 == 1:
            return 模型回應(工具呼叫清單=[{
                "id": "call_prefixed",
                "type": "function",
                "function": {"name": "default_api.read_file", "arguments": json.dumps({"path": "/Users/wujinan/Documents/testagent2/README.md", "limit": 1})},
            }])
        return 模型回應(文字="完成")
...
def test_runtime_接受_gemini_default_api_工具前綴(tmp_path):
    ...
    runtime = 代理執行階段(庫, 前綴工具名稱模型(), 模型名稱="fake", 供應商名稱="fake", 工作目錄="/Users/wujinan/Documents/testagent2")
