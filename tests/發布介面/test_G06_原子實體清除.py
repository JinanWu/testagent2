...
@pytest.mark.parametrize("階段", ["DELETE FROM endpoint_redactions", "DROP TRIGGER", "CREATE TRIGGER", "COMMIT"])
def test_任一刪除DDL重建或提交失敗均回滾資料與guard(資料庫, monkeypatch, 階段):
    with closing(sqlite3.connect(資料庫)) as 連線, 連線:
        _根(連線, "eligible", 0)
        _完整相依(連線, "eligible", 0)
        原guards = _guards(連線)
...
def test_BEGIN_IMMEDIATE阻擋競爭writer但既有讀snapshot一致(資料庫):
    with closing(sqlite3.connect(資料庫)) as 連線, 連線:
        _根(連線, "eligible", 0)
    reader = sqlite3.connect(資料庫, isolation_level=None)
    reader.execute("PRAGMA journal_mode=WAL")
    reader.execute("BEGIN")
    assert reader.execute("SELECT count(*) FROM endpoint_invocations").fetchone() == (1,)
    assert SQLite保存清除服務(str(資料庫)).清除(_秒(2026)).呼叫數 == 1
    assert reader.execute("SELECT count(*) FROM endpoint_invocations").fetchone() == (1,)
    reader.rollback()
    assert reader.execute("SELECT count(*) FROM endpoint_invocations").fetchone() == (0,)
    reader.close()
